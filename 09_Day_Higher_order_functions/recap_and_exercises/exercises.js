...
const name = countries1.map((country) => country.name);
const haveLand = [];
const haveIa = [];
const island = [];
const stan = [];
const filtered = name.forEach((item) => {
  if (
    (item.slice(-4) === "land" || item.includes("land")) &&
    item.slice(-6) !== "Island" &&
    !item.includes("Island")
  ) {
    haveLand.push(item);
  } else if (item.slice(-2) === "ia") {
    haveIa.push(item);
  } else if (item.slice(-6) === "Island" || item.includes("Islands")) {
    island.push(item);
  } else if (item.slice(-4) === "stan" || item.includes("stan")) {
    stan.push(item);
  }
});
...
// function fihrist() {
//   const alphabet = [];
//   const startLetter = name.filter((country) => country.startsWith(`${letter}`));
//   const letterCount = { letter: letter, count: startLetter.length };
//   alphabet.push(letterCount);
//   return alphabet;
// }
...
function getFirstTenCountries() {
  const firstTen = countries1.filter((_, index) => index < 10);
...
function getLastTenCountries() {
  const lastTen = countries1.filter(
    (_, index) => countries1.length - 11 < index
  );
  console.log(lastTen);
}
getLastTenCountries();
...
function mostUsedInitial(countries1) {
  const letterCount = {};
...
const result = mostUsedInitial(countries1);
console.log(result);
